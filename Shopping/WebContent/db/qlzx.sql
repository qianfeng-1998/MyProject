...
--内连接，多表查询
select a.id,a.title,b.userName,a.createTime from Bulletin a inner join UserInfo b on a.userId=b.id


--分页查询
select top 3 a.id,a.title,b.userName,a.createTime from Bulletin a 
		inner join UserInfo b on a.userId=b.id where a.id not in(select top ((1-1)*3) Bulletin.id from Bulletin order by Bulletin.id desc)  order by a.id  desc
